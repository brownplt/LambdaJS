...
toANF expr k =
    case expr of
      ...
      EOp a op args ->
        toANFValues args $ \vargs -> k $ Right $ BOp a op vargs
      EApp a func args -> 
      ...
      ESetRef a id e -> 
        toANFValue e $ \v -> k $ Right $ BSetRef a id  v
      ERef a e ->
        toANFValue e $ \v -> k $ Right $ BRef a v
      EDeref a e ->
        toANFValue e $ \v -> k $ Right $ BDeref a v
      EGetField a obj name -> 
        toANFValue obj $ \vobj ->
          toANFValue name $ \vname -> k $ Right $ BGetField a vobj vname
      EUpdateField a obj name val -> 
        toANFValue obj $ \vobj ->
          toANFValue name $ \vname -> 
            toANFValue val $ \vval -> k $ Right $ BUpdateField a vobj vname vval
      EDeleteField a obj name ->
        toANFValue obj $ \vobj ->
          toANFValue name $ \vname -> k $ Right $ BDeleteField a vobj vname
      ESeq a e1 e2 -> do
        e1' <- toANF e1 $ \vb -> return (toExp' a vb)
        e2' <- toANF e2 k
        case e1' of
      ...
      EFinally a body rest -> do
        body' <- toANF body k
        rest' <- toANF rest $ \vb -> return $ toExp' a vb
